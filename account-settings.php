...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['update_profile'])) {
        ...
        try {
                ...
                if ($user = $result->fetch_assoc()) {
                    if (password_verify($delete_password, $user['password'])) {
                        ...
                        try {
                            ...
                            $delete_activities = "DELETE FROM user_activities WHERE uid = ?";
                            $stmt = $db->prepareAndExecute($delete_activities, "i", [$user_id]);
                            
                            // Delete user appointments
                            $delete_appointments = "DELETE FROM appointments WHERE client_id = ?";
                            $stmt = $db->prepareAndExecute($delete_appointments, "i", [$user_id]);

                            // Delete user consultations
                            $delete_consultations = "DELETE FROM consultations WHERE uid = ?";
                            $stmt = $db->prepareAndExecute($delete_consultations, "i", [$user_id]);
                            
                            // Delete user
                            $delete_user = "DELETE FROM users WHERE uid = ?";
                            $stmt = $db->prepareAndExecute($delete_user, "i", [$user_id]);
            ...
            $delete_error = "An error occurred while deleting your account.";
...
<body class="user-settings-bg">
      ...
      <div class="content">
        ...
        <div id="settings" class="tab-content hidden">
          ...
          <div class="delete-account-section">
            ...
            <button type="button" class="delete-account-btn" onclick="showDeleteConfirmation()">Delete My Account</button>
          </div>
          ...
          <div id="deleteAccountModal" class="modal">
            <div class="modal-content">
              ...
              <ul>
                ...
                <li>Remove all your appointments and records</li>
                <li>Cannot be undone</li>
              </ul>
              <div class="modal-buttons">
                <button onclick="showPasswordConfirmation()" class="confirm-delete">Yes, I want to delete my account</button>
                <button onclick="closeDeleteModal()" class="cancel-delete">Cancel</button>
              </div>
            </div>
          </div>

          <!-- Second Confirmation Modal (Password) -->
              ...
              <form id="deleteAccountForm" method="post" action="">
                <div class="form-group">
                  ...
                  <div class="password-wrapper">
                    ...
                    <img src="https://cdn-icons-png.flaticon.com/512/565/565655.png" alt="Show" id="toggleDeletePassword" />
                  </div>
                </div>
                <div class="modal-buttons">
                  <button type="submit" name="delete_account" class="confirm-delete">Delete Account</button>
                  <button type="button" onclick="closePasswordModal()" class="cancel-delete">Cancel</button>
                </div>
              </form>
            </div>
          </div>
  ...
  <script>
    ...
    function showPasswordConfirmation() {
        ...
        const passwordModal = document.getElementById('passwordConfirmationModal');
        if (deleteModal && passwordModal) {
    ...
    // Setup delete password toggle
    setupToggle("toggleDeletePassword", "deletePassword");
